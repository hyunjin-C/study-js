...
[7. Objects](#objects)   
[8. Array](#array)   
[9. Array API 정리](#array-api-정리)   
[10. JSON의 개념과 활용 방법](#JSON의-개념과-활용-방법)
...
> **join과 toString의 차이점**   
> join과 toString 모두 배열을 문자열로 만들어주는데 toString과 달리 join을 사용하면 구분자(separator)를 지정할 수 있다.   
...
> join(separator?)과 split(separator, limit?)은 반대되는 개념   
> join은 array->string, split은 string->array   
...
> **splice를 쓸 수 없는 이유?**   
> splice는 array 배열 자체를 변환하고 제거하는 값을 반환하므로 새로운 배열에 반환값을 저장하는 형태로는 사용할 수 없다.      
> 따라서 반환값을 새로운 배열에 넣을 때는 slice를 이용해야 원하는 값이 나온다.   
...
-> 배열 안에 들어있는 모든 요소들을 callback 함수들에서 가공되어진 값으로 리턴되어진다.   
```javascript
...
> **some과 every의 활용**   
> - some: 하나라도 만족되는 조건이 있다면 true가 리턴된다.   
...
```
> result.sort((a + b) => b - a)는 큰 수부터 정렬된다.     
...
> **유의할 점!**     
> rabbit을 JSON으로 변환할 때 데이터만 변환되고 함수는 포함되지 않기 때문에 다시 JSON에서 object로 변환할 때도 함수가 포함되지 않으므로 그 object에는 jump라는 기능이 없다.   

```javascript
console.log(rabbit.birthDate.getDate());
console.log(obj.birthDate.getDate());
```
json으로 변환될 때 birthDate가 string으로 만들어졌기 때문에 이것을 다시 object로 가져올 때도 string으로 할당되어진다.
따라서 json으로 변환되었다가 다시 객체로 변환한 obj는 현재 string이기 때문에 obj.birthDate.getDate()는 객체로 접근하는 것이기 때문에 에러가 나게 된다.
에러가 나지 않도록 수정하려면 callback 함수를 사용해서 birthDate를 새로운 객체로 만들면 된다.

> **※ 참고**   
> **<JSON 관련 유용한 웹사이트>**   
> - 서버에서 요청했을 때 첫 번째로 받아 온 결과와 두 번째로 받아온 결과가 어떻게 다른지 비교할 수 있는 사이트(문제를 디버깅할 때): JSON Diff
> - 서버에서 받아 온 JSON을 붙여넣으면 가끔 format이 망가질 때가 있는데 이를 올바르게 만들어주는 사이트: JSON Beautifier
> - JSON 타입을 object로 확인해보고 싶을 때, JSON으로부터 Object가 어떻게 표기되어지는지 알 수 있는 사이트: JSON Parser
